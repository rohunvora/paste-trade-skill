 ...
 *
 * Usage:
 *   bun run scripts/stream-thought.ts --run-id <runId> "Found 4 theses: oil supply risk, gold safe haven, defense rally, Israel posture"
 */
...
if (!message) {
  console.error("Usage: bun run scripts/stream-thought.ts --run-id <runId> '<message>'");
  process.exit(1);
}
